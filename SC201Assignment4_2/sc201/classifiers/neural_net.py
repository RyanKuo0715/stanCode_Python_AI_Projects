...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        b1 = b1.reshape((1, H))
        K = np.dot(X, W1) + b1  # X(N, D) W1(D, H) b1(1, H)
        A = np.maximum(0, K)
        b2 = b2.reshape((1, C))
        F = np.dot(A, W2) + b2  # A(N, H) W2(H, C) b2(1, C)
        scores = F
        ...
        sigma[range(N), y] -= 1
        grads_classScores = sigma/N  # grads_classScores(N, C)
        grads['W2'] = np.dot(A.T, grads_classScores) # A(N, H) Scores(N, C) W2(H, C)
        grads['b2'] = np.sum(grads_classScores, axis=0)  # Scores(N, C) b2(1, C)
        grads_A1 = np.dot(grads_classScores, W2.T) # W2(H, C) Scores(N, C) A1(N, H)
        grads_K1 = grads_A1*np.where(K>0, 1, 0) # A1(N, H) K(N, H) K1(N, H)
        grads['W1'] = np.dot(X.T, grads_K1) # X(N, D) K1(N, H) W1(D, H)
        grads['b1'] = np.sum(grads_K1, axis=0) # K1(N, H) b1(1, H)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            batch_values = np.random.randint(num_train, size=batch_size)
            X_batch = X[batch_values]
            y_batch = y[batch_values]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def predict(self, X):
        ...
        A = np.maximum(0, K)
        F = np.dot(A, self.params['W2']) + self.params['b2']
